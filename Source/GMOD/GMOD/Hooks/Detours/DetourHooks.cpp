...
bool ShouldRecharge()
{
	if (GetAsyncKeyState(VK_XBUTTON2) && gGlobalInfo.CurrentCharge < 48)
	{
	//	if (!(gInts.GlobalVars->tickcount % 4)) return false;
		return true;
	}
...
void DetourHooks::CLMove::Hook(float accumulated_extra_samples, bool bFinalTick)
{
	auto Original = (CLMoveFn)Detour.GetOriginal();
