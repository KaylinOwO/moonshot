...
void C_Framework::Setup()
{
	...
	{
		gDraw.StringCenter(FONT_MENU, gFramework.MenuPos.x - gMenu.TabArr[gMenu.CurTab].size() - 25, gFramework.MenuPos.y + 2 + (15 * gFramework.CurrentPosition) + 2, Color_t{ 90, 14, 255, 255 }, ">>");
		gDraw.StringCenter(FONT_MENU, gFramework.MenuPos.x + gMenu.TabArr[gMenu.CurTab].size() + 25, gFramework.MenuPos.y + 2 + (15 * gFramework.CurrentPosition) + 2, Color_t{ 90, 14, 255, 255 }, "<<");
	}
	else
	...
	for (int i = 0; i < gFramework.TotalItems; ++i) {
		...
		if (!strncmp(Items[i].Name.c_str(), "[+]", 3) || !strncmp(Items[i].Name.c_str(), "[-]", 3)) {
			...
			case 1: /*INT*/
				...
				{
					gDraw.String(FONT_MENU, gFramework.MenuPos.x + 10, gFramework.MenuPos.y + 2 + (15 * i) + 4, Color_t{ 255,255,255,255 }, Items[i].Name.c_str());
					gDraw.String(FONT_MENU, gFramework.MenuPos.x + 150, gFramework.MenuPos.y + 2 + (15 * i) + 4, Color_t{ 255,255,255,255 }, (Items[i].Mode != "") ? Items[i].Mode : "%i", *Items[i].m_Int);
				}
