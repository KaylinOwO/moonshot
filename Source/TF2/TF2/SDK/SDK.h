...
namespace Util
{
	...
	inline bool WeaponCanHeadshot(C_BaseEntity* pLocal)
	{
		auto pWeapon = pLocal->GetActiveWeapon(); if (!pWeapon) return false;

		if (pLocal->GetTFClass() == TF_CLASS_SNIPER && pLocal->GetActiveWeapon()->GetSlot() == 0 /*Primary*/)
			return true;

		if (pLocal->GetTFClass() == TF_CLASS_SPY && (pWeapon->GetItemDefinitionIndex() == Spy_m_TheAmbassador || pWeapon->GetItemDefinitionIndex() == Spy_m_FestiveAmbassador))
			return true;
	...
	{
		if (!pLocal || pLocal->IsDormant() || !pLocal->IsAlive()) return;
